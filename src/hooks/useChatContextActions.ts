...
const useChatContextActions = (
  ...
  return useMemo(() => {
    ...
    const actionQuickPreview = !isSavedDialog &&
      !chat.isForum && {
    ...
    const actionAiAnalysis = {
      title: "AI Анализ",
      icon: "bots",
      handler: () => {
        if (isSavedDialog) {
          openChat(
            { id: currentUserId!, shouldReplaceHistory: true },
            { forceOnHeavyAnimation: true },
          );
          toggleAiAgentForChat({ chatId: currentUserId!, isEnabled: true });
        } else {
          openChat(
            { id: chat.id, shouldReplaceHistory: true },
            { forceOnHeavyAnimation: true },
          );
          toggleAiAgentForChat({ chatId: chat.id, isEnabled: true });
        }
      },
    };
    ...
    const actionMaskAsRead =
      chat.unreadCount ||
      chat.hasUnreadMark ||
      Object.values(topics || {}).some(({ unreadCount }) => unreadCount)
        ? {
    ...
    const actionMarkAsUnread =
      !(chat.unreadCount || chat.hasUnreadMark) && !chat.isForum
        ? {
            ...
            icon: "unread",
            handler: () => markChatUnread({ id: chat.id }),
          }
        : undefined;
    ...
    const canReport =
      handleReport && !user && (isChatChannel(chat) || isChatGroup(chat));
