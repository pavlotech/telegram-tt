...
import { selectCurrentMessageList, selectWebPageFromMessage } from "./messages";
...
export function selectIsMediaViewerOpen<T extends GlobalState>(
  ...
  return Boolean(
    standaloneMedia ||
    (chatId && (isAvatarView || messageId || isSponsoredMessage)),
  );
}
...
export function selectRightColumnContentKey<T extends GlobalState>(
  ...
  const chatId = messageList?.chatId;
  ...
  if (isAiAgentEnabled) {
    return RightColumnContent.AiAgent;
  }
                  ...
                  : tabState.monetizationStatistics
                    ...
                    : tabState.stickerSearch.query !== undefined
                      ...
                      : tabState.gifSearch.query !== undefined
                        ...
                        : tabState.newChatMembersProgress !==
                            NewChatMembersProgress.Closed
                          ? RightColumnContent.AddingMembers
                          : tabState.chatInfo.isOpen &&
                              tabState.messageLists.length
...
export function selectThemeValues<T extends GlobalState>(
  global: T,
  themeKey: ThemeKey,
) {
...
export function selectIsForumPanelOpen<T extends GlobalState>(
  ...
  return (
    Boolean(tabState.forumPanelChatId) &&
    (tabState.globalSearch.query === undefined ||
      Boolean(tabState.globalSearch.isClosing))
  );
}
...
export function selectCanAutoPlayMedia<T extends GlobalState>(
  global: T,
  ...
  const canAutoPlayVideos = selectPerformanceSettingsValue(
    global,
    "autoplayVideos",
  );
  const canAutoPlayGifs = selectPerformanceSettingsValue(
    global,
    "autoplayGifs",
  );
...
export function selectCanAnimateSnapEffect<T extends GlobalState>(global: T) {
  return (
    ...
    selectPerformanceSettingsValue(global, "snapEffect")
  );
}

export function selectIsContextMenuTranslucent<T extends GlobalState>(
  global: T,
) {
...
export function selectPeerProfileColor<T extends GlobalState>(
  global: T,
...
export function selectTabSelectedAuctionGiftId<T extends GlobalState>(
  ...
  const giftModalAuctionGiftId =
    selectedGift &&
    "id" in selectedGift &&
    selectedGift.type === "starGift" &&
    selectedGift.isAuction
      ? selectedGift.id
      : undefined;

  return (
    tabState.giftAuctionModal?.auctionGiftId ||
...
export function selectHasAnySelectedAuction<T extends GlobalState>(
  global: T,
  giftId: string,
) {
...
export function selectShouldRemoveGiftAuction<T extends GlobalState>(
  global: T,
  giftId: string,
) {
