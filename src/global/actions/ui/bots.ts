...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    const { webApp, tabId = getCurrentTabId() } = payload;
...
addActionHandler(
  "updateWebApp",
  (global, actions, payload): ActionReturnType => {
    const { key, update, tabId = getCurrentTabId() } = payload;
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    if (!hasOpenedWebApps(global, tabId))
      return replaceIsWebAppModalOpen(global, false, tabId);
      ...
      global,
      {
        webApps: {
          ...
          activeWebAppKey: openedWebAppsCount
            ? openedWebAppsKeys[openedWebAppsCount - 1]
            : undefined,
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    global = removeWebAppFromOpenList(
      global,
      key,
      skipClosingConfirmation,
      tabId,
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    const shouldShowConfirmation =
      ...
      !selectSharedSettings(global).shouldSkipWebAppCloseConfirmation &&
      hasOpenedMoreThanOneWebApps(global, tabId);
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    const newPlannedEvents = webApp.plannedEvents
      ? [...webApp.plannedEvents, event]
      : [event];
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    const { bot, filter, startParam, tabId = getCurrentTabId() } = payload;
    const currentChatId = selectCurrentMessageList(global, tabId)?.chatId;

    const supportedFilters = bot.attachMenuPeerTypes?.filter(
      (type): type is ApiChatType => type !== "self" && filter.includes(type),
    );
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    const { bot, webAppKey, tabId = getCurrentTabId() } = payload;
